...
**Status:** ✅ Complete
**Priority:** 🟡 Medium
**Effort:** M (Medium - 7 hours actual)
...
- [x] Backend: Extend `DonorMergeService` to reassign donations
- [x] Backend: Extend `DonorMergeService` to reassign sponsorships
- [x] Backend: Reassign AFTER creating merged donor (need new ID)
- [x] Backend: Preserve all donation/sponsorship data (amount, date, stripe fields)
- [x] Backend: Return counts in merge response: `donations_reassigned`, `sponsorships_reassigned`
- [x] Backend: Transaction ensures atomicity (rollback on failure)
- [x] RSpec: 5 tests for reassignment logic

#### Phase 2: Future Stripe Import Auto-Redirect ✅
- [x] Backend: Modify `DonorService.find_or_update_by_email` to check `stripe_customer_id`
- [x] Backend: Follow merge chain if donor has `merged_into_id`
- [x] Backend: Return `redirected: true` flag when merge redirect happens
- [x] Backend: Update `StripePaymentImportService` to pass `stripe_customer_id`
- [x] RSpec: 4 tests for stripe_customer_id lookup + merge redirect
- [x] RSpec: 1 integration test for import service

#### Phase 3: Frontend & E2E ✅
- [ ] Frontend: Display reassignment counts in merge success message (deferred - backend returns counts)
- [x] Cypress: Verify donations visible under merged donor after merge
- [x] Cypress: Verify sponsorships visible under merged donor after merge
...
- [x] All donations reassigned during merge
- [x] All sponsorships reassigned during merge
- [x] Future Stripe imports use merged donor (auto-redirect)
- [x] Manual donations cannot select merged donors (already working ✅)
- [x] All RSpec tests pass (10 new tests: 5 DonorMergeService + 4 DonorService + 1 integration)
- [x] Cypress E2E tests pass (2 tests: donations + sponsorships)
- [x] PaperTrail audit trail preserved
...
- TICKET-071: Stripe CSV Batch Import Task 🟡 (benefits from auto-redirect)

### Related Commits
- `70d9695` - backend: auto-reassign donations/sponsorships after donor merge (Phase 1 & 2)
- `70814eb` - frontend: add E2E tests for donor merge reassignment (Phase 3)
