...
describe('ChildList', () => {
  ...
  it('shows add sponsor icon for active children', () => {
    render(
      <ChildList
        ...
        onEdit={jest.fn()}
        onArchive={jest.fn()}
        onAddSponsor={jest.fn()}
      />
    );

    const addSponsorButtons = screen.getAllByRole('button', {
      name: /add sponsor/i,
    });
  ...
  it('shows empty state message when no children', () => {
    render(
      <ChildList children={[]} onEdit={jest.fn()} onArchive={jest.fn()} />
    );
  ...
  it('displays single active sponsor correctly', () => {
    ...
    expect(
      screen.getByText(/sponsored by: john doe \(\$50\.00\/mo\)/i)
    ).toBeInTheDocument();
  });
  ...
  it('displays multiple active sponsors comma-separated', () => {
    ...
    expect(
      screen.getByText(
        /sponsored by: john doe \(\$50\.00\/mo\), jane smith \(\$75\.00\/mo\), bob johnson \(\$100\.00\/mo\)/i
      )
    ).toBeInTheDocument();
  });
